...
class TestBoard(unittest.TestCase):
    ...
    def test_board_setup(self) -> None:
        ...
        sum_layout_crow: int = sum(1 for value in board.layout.values() if value is CellType.CROW)
        sum_layout_wolf: int = sum(1 for value in board.layout.values() if value is CellType.WOLF)

        sum_content_crow: int = sum(1 for value in board.content.values() if value is ControlMarker)
        sum_content_wolf: int = sum(1 for value in board.content.values() if value is ControlMarker)
